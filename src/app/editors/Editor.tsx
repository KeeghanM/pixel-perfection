...
export default function Editor(props: Props) {
    ...
    const docChangedField = StateField.define({
        create() { return false; },
        update(value, tr) {
            if (tr.docChanged) return docChangedEffect.of(tr.docChanged);
            for (let effect of tr.effects) {
                if (effect.is(docChangedEffect)) return true;
            }
            return value;
        },
        effects: [docChangedEffect]
    });

    // Update the editor when the challenge changes
    createEffect(() => {
        const newText = props.type === "HTML" ? challenge().HTML : challenge().wrongCSS
        if (editor) {
            editor.dispatch({
    ...
    onMount(() => {
        ...
        const setTheme = () => {
            return document.documentElement.classList.contains("dark") ? oneDark : solarizedLight;
        }
        const createEditor = (theme:any) => {
            ...
            editor = new EditorView({
                state: EditorState.create({
                    doc: currentText,
                    extensions: [basicSetup, props.Lang(),
                        docChangedField,
    ...
    return (
        <div ref={editorRef} class="w-full h-full max-h-full"></div>
    )
}